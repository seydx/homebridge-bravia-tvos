...
class Bravia {
  constructor(platform, context, waitUntilAuthenticated) {
    ...
    this.url = 'http://' + context.ip + ':' + context.port;
  ...
  _handleAuth(callback){
    if(this.authenticated){
      ...
      if(this.authCount >= 12){
        ...
        callback(self.name + ': Can not authenticate!', null);
  ...
  _refreshToken() {
    ...
    return new Promise((resolve, reject) => {
      ...
      if(this.context.psk){
        ...
        this.headers['X-Auth-PSK'] = this.context.psk;
        resolve(true);
        ...
        this._refreshToken().then(() => {
          ...
          axios({
            ...
            if(response.data.result){
              ...
              if(isObject(response.data) && response.data.includes('<?xml version="1.0"?>')){
