  ...
  if (this.config.tvs && this.config.tvs.length) {
    this.config.tvs.forEach((tv) => {
      ...
      if (!tv.name) {
        ...
        Logger.warn(
          'There is no psk or application name/uuid configured for this tv. This tv will be skipped.',
          tv.name
        );
        error = true;
      }
      ...
      if (!error) {
        ...
        if (this.devices.has(uuid)) {
          ...
          tv.displayOrder = tv.displayOrder
            .map((catagory) => {
              if (
                validCatagories.includes(catagory) &&
                !addedCatagories.includes(catagory) &&
                addedCatagories.length <= 4
              ) {
          ...
          if (tv.displayOrder.length < 4) {
            validCatagories.forEach((catagory) => {
              if (!tv.displayOrder.includes(catagory)) tv.displayOrder.push(catagory);
            });
          }
...
BraviaOSPlatform.prototype = {
  didFinishLaunching: async function () {
    ...
    this.accessories.forEach((accessory) => {
      ...
      try {
        if (!tv) this.removeAccessory(accessory);
      } catch (err) {
  ...
  getAccessoryFromCache: async function (accessory) {
    try {
      ...
      if (cache) {
        ...
        if (accessoryCache && accessoryCache.cache) return accessoryCache.cache;
      }
    } catch (err) {
