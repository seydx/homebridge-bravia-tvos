...
class tvAccessory {
  ...
  async getService() {
    ...
    if (!service) {
      ...
      service
        .setCharacteristic(
          ...
          this.accessory.context.cache.tvName || this.accessory.displayName
        )
        .setCharacteristic(
    ...
    service.getCharacteristic(this.api.hap.Characteristic.ConfiguredName).on('set', (name, callback) => {
      this.accessory.context.cache.tvName = name;
      callback(null);
    });

    service.getCharacteristic(this.api.hap.Characteristic.Active).on('set', this.setPower.bind(this));

    service
      .getCharacteristic(this.api.hap.Characteristic.ActiveIdentifier)
      .on('set', this.setActiveIdentifier.bind(this));

    service.getCharacteristic(this.api.hap.Characteristic.RemoteKey).on('set', this.setRemoteKey.bind(this, null));

    service
      .getCharacteristic(this.api.hap.Characteristic.PowerModeSelection)
      .on('set', this.setRemoteKey.bind(this, 'SETTINGS'));

    speakerService.getCharacteristic(this.api.hap.Characteristic.VolumeSelector).on('set', this.setVolume.bind(this));
  ...
  async poll() {
    ...
    let speakerAccessory = this.accessories.find(
      (accessory) => accessory.displayName === this.accessory.displayName + ' Speaker'
    );
    ...
    try {
      ...
      speakerAccessory = this.accessories.find(
        (accessory) => accessory.displayName === this.accessory.displayName + ' Speaker'
      );
  ...
  async setVolume(value, callback) {
    ...
    try {
      ...
      if (target === 'other') {
        ...
        try {
          await this.bravia.audio.invoke('setAudioVolume', '1.0', {
            target: target,
            volume: volumeLevel < 0 ? volumeLevel.toString() : '+' + volumeLevel.toString(),
          });
        } catch (err) {
          if (!(err instanceof Error) && err.includes(40801)) {
            Logger.warn(
              'Can not ' + (value ? 'reduce' : 'increase') + ' volume. Volume Level out of range.',
      ...
      Logger.error(
        'An error occured during ' + (value ? 'decreasing' : 'increasing') + ' volume!',
        this.accessory.displayName
      );
      this.handleError(err);
    }
    ...
    if (name) {
      try {
        switch (this.fetchedInputs[state].type) {
          case 'input':
            Logger.info(
              'Changing input source to ' + name + ' (' + this.fetchedInputs[state].uri + ')',
              this.accessory.displayName
            );
          ...
          case 'channel':
            Logger.info(
              'Changing channel to ' + name + ' (' + this.fetchedInputs[state].uri + ')',
              this.accessory.displayName
            );
            await this.bravia.avContent.invoke('setPlayContent', '1.0', { uri: this.fetchedInputs[state].uri });
            break;
          case 'application':
            Logger.info(
              'Changing application to ' + name + ' (' + this.fetchedInputs[state].uri + ')',
              this.accessory.displayName
            );
          ...
          case 'ircc':
            Logger.info(
              'Sending "' + name + '" command (' + this.fetchedInputs[state].value + ')',
              this.accessory.displayName
            );
            await this.bravia.send(this.fetchedInputs[state].value);
            break;
          case 'macro':
            Logger.info(
              'Sending [' +
                this.fetchedInputs[state].commands.toString() +
                '] commands with ' +
                this.fetchedInputs[state].delay +
                'ms delay',
              this.accessory.displayName
            );
      ...
      Logger.warn(
        'Can not change source. ' + name + ' not found! Try updating inputs and restart homebridge!',
        this.accessory.displayName
      );
    }
  ...
  async updateInputs() {
    ...
    //fetch all channels
    let channelSources = ['tv:dvbt', 'tv:dvbc', 'tv:dvbs', 'tv:isdbt', 'tv:isdbs', 'tv:isdbc', 'tv:analog'];
    let allChannels = [];
    for (const source of channelSources) {
      Logger.debug('Fetching all channels for ' + source, this.accessory.displayName);
      for (let i = 0; i <= 10; i++) {
        //max channels 10 * 200 for source
        try {
          Logger.debug('Fetching ' + (i + 1) + '. list of channels', this.accessory.displayName);
          let channels = await this.bravia.avContent.invoke('getContentList', '1.2', {
            stIdx: i * 200,
            cnt: i * 200 + 200,
            source: source,
          });
    ...
    try {
      ...
      Logger.debug(
        'All inputs fetched! Stored to ' + (this.api.user.storagePath() + '/bravia/' + this.fileName),
        this.accessory.displayName
      );
    } catch (err) {
      Logger.error(
        'An error occured during storing inputs! Restart homebridge or try again to update inputs.',
        this.accessory.displayName
      );
      Logger.error(err);
    }
          ...
          if (channelInfo) {
            ...
            if (uri.includes(channelSource) && config.index + 1 === channelIndex && !inputSourceNames.includes(uri)) {
              ...
              if (inputSourceNames.includes(uri))
                Logger.warn(
                  'Skip ' + channelTitle + ' (' + channelIndex + '). Already in use.',
                  this.accessory.displayName
                );
            }
          } else {
  ...
  addInputServices(service, inputSources) {
    inputSources.forEach((input) => {
      ...
      InputService.setCharacteristic(this.api.hap.Characteristic.Identifier, input.identifier)
        .setCharacteristic(
          this.api.hap.Characteristic.ConfiguredName,
          this.accessory.context.cache.inputs[input.name].inputName || input.name
        )
        .setCharacteristic(
        ...
        .setCharacteristic(
          this.api.hap.Characteristic.CurrentVisibilityState,
          this.accessory.context.cache.inputs[input.name].visibility || 0
        )
        .setCharacteristic(
          this.api.hap.Characteristic.TargetVisibilityState,
          this.accessory.context.cache.inputs[input.name].visibility || 0
        )
      ...
      InputService.getCharacteristic(this.api.hap.Characteristic.CurrentVisibilityState).on('get', (callback) => {
      ...
      InputService.getCharacteristic(this.api.hap.Characteristic.ConfiguredName).on('set', (name, callback) => {
  ...
  getIndex(target, name, index) {
    ...
    if (target) {
      if (index !== undefined) {
        let availableInputs = this.fetchedInputs.filter(
          (input) => input && input[target] && input[target].includes(name)
        );
        let selectedInput = availableInputs[index];

        identifier = this.fetchedInputs
          .map((input, index) => {
  ...
  handleError(err) {
    ...
    if (err instanceof Error) {
      if (err.code && err.code === 'ECONNABORTED') {
        Logger.debug('Timeout of ' + this.accessory.context.config.timeout + 's exceeded!', this.accessory.displayName);
      } else if (
        err.code &&
        ...
        Logger.debug('Can not reach TV!', this.accessory.displayName);
      } else {
        Logger.error(err);
      }
    } else if (Array.isArray(err)) {
