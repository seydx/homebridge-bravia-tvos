...
class speakerAccessory {
  ...
  async setVolume(serviceCharacteristic, onCharacteristic, volumeCharacteristic, value, callback) {
    ...
    let tvAccessory = this.accessories.find(
      (accessory) => accessory.displayName + ' Speaker' === this.accessory.displayName
    );
    let passed = false;
    ...
    if (passed) {
      ...
      this.waitForEndValue = setTimeout(async () => {
        ...
        if (onCharacteristic !== this.api.hap.Characteristic.Mute) {
          this.accessory.getService(serviceCharacteristic).getCharacteristic(onCharacteristic).updateValue(false);
        }
      }, 1000);
    }
  }
